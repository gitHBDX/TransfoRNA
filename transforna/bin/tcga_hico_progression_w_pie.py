...
def plot_heatmap_overlap_models_ensemble(models,mc_stats,novel_resid,mcs_predicted_by_only_one_model,what_to_plot='overlap'):
    ...
    #plot a heatmap with models as rows and mc classes as columns
    import plotly.graph_objects as go
    import numpy as np
    import plotly.express as px
    #use 
...
#UNCOMMENT TO COMPUTE BEFORE AND AFTER PER MC: Fig 8
...
#%%%
#here i want to copy the full_df to a new df and add MC_Lables as a column by mapping it using the  mapping_dict
#full_df = predict_transforna_all_models(sequences=seqs_non_hico_id,trained_on='full')
inspect_model = True
if inspect_model:
...
fig = go.Figure()
fig.add_trace(go.Bar(
    x=hicos_mc_before_id_stats.index,
    y=hicos_mc_before_id_stats.values,
    name='KBA',
    marker_color='rgb(31, 119, 180)',
    opacity = 0.5
))
fig.add_trace(go.Bar(
    x=hicos_mc_after_id_stats.index,
    y=hicos_mc_after_id_stats.values,
    name='After ID',
    marker_color='rgb(31, 119, 180)',
    opacity=0.75
))
fig.add_trace(go.Bar(
    x=hicos_mc_after_full_stats.index,
    y=hicos_mc_after_full_stats.values,
    name='After FULL',
    marker_color='rgb(31, 119, 180)',
    opacity=1
))
...
#order the x axis to start with 'rRNA','tRNA', 'snRNA', 'snoRNA', 'protein_coding'
fig.update_xaxes(categoryorder='array',categoryarray=['rRNA','tRNA', 'snoRNA', 'protein_coding','snRNA','miRNA','miscRNA','lncRNA','piRNA','YRNA','vtRNA'])
#the first five major classes are ID and the rest are FULL
#add a vrect to separate ID from FULL
fig.add_vrect(x0=-0.5, x1=4.5, fillcolor="yellow", opacity=0.1, line_width=0,annotation_text = 'Used for ID+FULL',annotation_position='top right')
fig.add_vrect(x0=4.55, x1=10.5, fillcolor='green', opacity=0.1, line_width=0,annotation_text = 'Used for FULL',annotation_position='top right')
# legend should be outside the plot
...
fig.show()
#save figure as html and as svg
pio.write_html(fig, file='progression_hicos_per_mc.html', auto_open=True)
fig.write_image("progression_hicos_per_mc.svg")
...
# %%
#plot a pie chart with the percentages of HICO, LOCO, NA before ID, after ID, after FULL
import plotly.graph_objects as go
import plotly.io as pio
import plotly.express as px
#LOCO should be light green, HICO should be dark green and NA should be red
color_mapping = {}
...
# %%
#color the keys starting in dark green if they start with 'HICO' else light green
color_mapping = {}
...
#ordeer keys starting with keys starting with 'HICO' first
...
#plot the pie chart with the order of the keys starting with 'HICO' first
